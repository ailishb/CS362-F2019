...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        mineEffect(choice1, choice2, state, handPos, currentPlayer);
    ...
    case minion:
        minionEffect(choice1, choice2, state, handPos, currentPlayer);
    ...
    case ambassador:
        ambassadorEffect(choice1, choice2, state, handPos, currentPlayer);
...
// Begin refactored card effect definitions
int baronEffect(int choice1, struct gameState *state, int currentPlayer) 
{
    int card_discarded = 0; // Flag for discard set
    ...
    if (choice1 > 0) { // Boolean true or going to discard an estate
        ...
        while(card_discarded == 0) {
            if (state->hand[currentPlayer][p] == estate) { // Found an estate card!
                ...
                card_discarded = -1; // Card cannot be discarded, exit loop
            }
            else {
                p++; // Next card
            }
